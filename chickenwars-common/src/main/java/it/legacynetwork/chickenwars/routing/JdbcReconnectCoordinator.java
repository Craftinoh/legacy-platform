...
public final class JdbcReconnectCoordinator implements ReconnectCoordinator {
    ...
    @Override public CompletionStage<Void> forget(final UUID player){return CompletableFuture.runAsync(()->{try(Connection c=dataSource.getConnection();PreparedStatement p=c.prepareStatement("DELETE FROM cw_reconnect_sessions WHERE player_id=?")){p.setString(1,player.toString());p.executeUpdate();}catch(SQLException e){throw new PersistenceException("Cleanup reconnect giocatore fallito",e);}},executor);}
