...
public final class ChickenWarsNetworkRuntime {
    ...
    public void startHeartbeat(ArenaManager arenas){heartbeat=new InstanceHeartbeatTask(arenas,routing().getPublisher(),config.getString("routing.instance-id",""),config.getString("routing.server-name",""));heartbeat.start(plugin);
        // L'esito del rientro torna al proxy sul canale dedicato: e' cio' che
        // distingue un rifiuto di ChickenWars da un errore di connessione.
        plugin.getServer().getMessenger().registerOutgoingPluginChannel(plugin,it.legacynetwork.chickenwars.routing.RejoinVerdictCodec.CHANNEL);
        arrivals=new ReservationArrivalService(arenas,routing().getCoordinator(),persistence.getProfiles(),messages,mainThread,new it.legacynetwork.chickenwars.routing.BukkitRejoinVerdictSink(plugin,new it.legacynetwork.chickenwars.routing.BukkitRejoinVerdictSink.OnlinePlayerLookup(){@Override public org.bukkit.entity.Player find(java.util.UUID id){return org.bukkit.Bukkit.getPlayer(id);}}));plugin.getServer().getMessenger().registerIncomingPluginChannel(plugin,"BungeeCord",arrivals);plugin.getServer().getPluginManager().registerEvents(arrivals,plugin);}
    public void close(){if(heartbeat!=null){heartbeat.stop();heartbeat=null;}if(arrivals!=null){arrivals.clear();plugin.getServer().getMessenger().unregisterIncomingPluginChannel(plugin,"BungeeCord",arrivals);org.bukkit.event.HandlerList.unregisterAll(arrivals);arrivals=null;}lobby.clear();persistence.close();plugin.getServer().getMessenger().unregisterOutgoingPluginChannel(plugin,"BungeeCord");plugin.getServer().getMessenger().unregisterOutgoingPluginChannel(plugin,it.legacynetwork.chickenwars.routing.RejoinVerdictCodec.CHANNEL);}
