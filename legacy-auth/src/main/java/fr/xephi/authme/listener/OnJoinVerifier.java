...
public class OnJoinVerifier implements Reloadable {
    ...
    public boolean refusePlayerForFullServer(PlayerLoginEvent event) {
        ...
        if (nonVipPlayer != null) {
            // The message is shown to the kicked player, so it must be resolved in their language.
            nonVipPlayer.kickPlayer(messages.retrieveSingle(nonVipPlayer, MessageKey.KICK_FOR_VIP));
            event.allow();
            return false;
        } else {
