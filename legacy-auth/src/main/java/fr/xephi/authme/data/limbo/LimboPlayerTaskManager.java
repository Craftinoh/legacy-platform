...
class LimboPlayerTaskManager {
    ...
    void registerMessageTask(Player player, LimboPlayer limbo, LimboMessageType messageType) {
        ...
        if (interval > 0) {
            // Only the key and its non-localized replacements are handed to the task: it translates
            // them on every run, so a language change takes effect on the next message.
            MessageTask messageTask = new MessageTask(player, messages, result.messageKey, result.args);
    ...
    void registerTimeoutTask(Player player, LimboPlayer limbo) {
        ...
        if (timeout > 0) {
            BukkitTask task = bukkitService.runTaskLater(
                new TimeoutTask(player, messages, playerCache), timeout);
