...
public class HelpProvider implements Reloadable {
    ...
    private List<String> buildHelpOutput(CommandSender sender, FoundCommandResult result, int options) {
        ...
        // Help is written for one recipient, so every text below is resolved in their language.
        final String language = localeResolver.getLanguage(sender);
        ...
        CommandDescription command =
            helpMessagesService.buildLocalizedDescription(result.getCommandDescription(), language);
     ...
     * @param language the language to write the info in
     */
    ...
    private void addPermissionsInfo(CommandDescription command, CommandSender sender, List<String> lines,
        ...
        lines.add(ChatColor.GOLD + helpMessagesService.getMessage(HelpMessage.DEFAULT, language) + ": "
            ...
            + helpMessagesService.getMessage(defaultPermission, language) + addendum);
