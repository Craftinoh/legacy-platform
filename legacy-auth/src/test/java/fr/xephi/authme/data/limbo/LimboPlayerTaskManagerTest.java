...
public class LimboPlayerTaskManagerTest {
    ...
    public void shouldRegisterMessageTask() {
        ...
        // The message must NOT be translated here: MessageTask resolves it on every run so that a
        // language change takes effect without the task being recreated.
    ...
    public void shouldInitializeMessageTaskWithCaptchaMessage() {
        ...
        // then
        assertThat(limboPlayer.getMessageTask(), not(nullValue()));
        // The captcha code is captured as a non-localized replacement; the text itself is only
        // translated when the task runs.
    ...
    public void shouldRegisterTimeoutTask() {
        ...
        // The kick message is translated when the task fires, not when it is scheduled.
        verifyNoInteractions(messages);
    }
