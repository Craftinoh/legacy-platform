...
public class HelpProviderTest {
    ...
    public void shouldShowChildren() {
        ...
        CommandDescription command = getCommandWithLabel(commands, "authme");
        FoundCommandResult result = newFoundResult(command, Collections.singletonList("authme"));
        given(helpMessagesService.getDescription(eq(getCommandWithLabel(commands, "authme", "login")), any()))
            .willReturn("Command for login [localized]");
        given(helpMessagesService.getDescription(eq(getCommandWithLabel(commands, "authme", "register")), any()))
    ...
    public void shouldDisableSectionsWithEmptyTranslations() {
        // given
        given(helpMessagesService.getMessage(eq(HelpSection.DETAILED_DESCRIPTION), any())).willReturn("");
        given(helpMessagesService.getMessage(HelpSection.DETAILED_DESCRIPTION)).willReturn("");
        given(helpMessagesService.getMessage(eq(HelpSection.ALTERNATIVES), any())).willReturn("");
        given(helpMessagesService.getMessage(HelpSection.ALTERNATIVES)).willReturn("");
        given(helpMessagesService.getMessage(eq(HelpSection.PERMISSIONS), any())).willReturn("");
    ...
    public void shouldNotReturnAnythingForAllDisabledSections() {
        // given
        given(helpMessagesService.getMessage(eq(HelpSection.COMMAND), any())).willReturn("");
    ...
    public void shouldSkipEmptyHeader() {
        // given
        given(helpMessagesService.getMessage(eq(HelpMessage.HEADER), any())).willReturn("");
        CommandDescription command = getCommandWithLabel(commands, "authme", "register");
        FoundCommandResult result = newFoundResult(command, Collections.singletonList("authme"));
    ...
    private static void setDefaultHelpMessages(HelpMessagesService helpMessagesService) {
        given(helpMessagesService.buildLocalizedDescription(any(CommandDescription.class), any()))
        ...
        for (HelpMessage key : HelpMessage.values()) {
            ...
            given(helpMessagesService.getMessage(eq(key), any()))
        ...
        for (DefaultPermission permission : DefaultPermission.values()) {
            ...
            given(helpMessagesService.getMessage(eq(permission), any()))
                .willReturn(text.substring(0, 1).toUpperCase(Locale.ROOT) + text.substring(1));
        }
        for (HelpSection section : HelpSection.values()) {
            String text = section.name().replace("_", " ").toLowerCase(Locale.ROOT);
            String message = text.substring(0, 1).toUpperCase(Locale.ROOT) + text.substring(1);
            given(helpMessagesService.getMessage(eq(section), any())).willReturn(message);
            // Whether a section is enabled at all is decided on the configured language, so the
            // single-argument overload is used for that and must be stubbed too.
            given(helpMessagesService.getMessage(section)).willReturn(message);
        }
    }
