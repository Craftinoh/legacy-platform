...
public class MessagesIntegrationTest {
    ...
    public void setUpMessages() throws IOException {
        ...
        messages = new Messages(messagesFileHandler, createSingleLanguageResolver());
    }
    ...
    /**
     * Returns a resolver that answers the single language this test operates on, so that the
     * behaviour verified here is the same as before the per-player language feature was added.
     *
     * @return a locale resolver always yielding the test language
     */
    private LocaleResolver createSingleLanguageResolver() {
        LocaleResolver localeResolver = mock(LocaleResolver.class);
        given(localeResolver.getLanguage(any(CommandSender.class))).willReturn("test");
        given(localeResolver.getLanguage(any(UUID.class))).willReturn("test");
        given(localeResolver.getConsoleLanguage()).willReturn("test");
