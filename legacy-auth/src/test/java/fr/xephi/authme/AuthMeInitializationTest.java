...
public class AuthMeInitializationTest {
    ...
    public void shouldInitializeAllServices() {
        ...
        // The language services must come up even though neither LegacyLobby nor language-common
        // is on the classpath: the fork must never require the Apteris modules to start.
        assertThat(injector.getIfAvailable(LocaleResolver.class), not(nullValue()));
        assertThat(injector.getIfAvailable(NetworkLanguageProviderHook.class), not(nullValue()));
        assertThat(injector.getIfAvailable(ClientLocaleProvider.class), not(nullValue()));
        // With no provider registered, resolution falls through to the configured language.
        assertThat(injector.getSingleton(LocaleResolver.class).getLanguage(UUID.randomUUID()),
            not(nullValue()));
    }
