...
public class PlayerListenerTest {
    ...
    public void shouldKickPreLoginLowestUnresolvedHostname() {
        ...
        given(messages.retrieveSingle(uniqueId, name, MessageKey.KICK_UNRESOLVED_HOSTNAME))
            .willReturn("Unresolved hostname");
    ...
    public void shouldAbortPlayerJoinForInvalidName() throws FailedVerificationException {
        ...
        // Pre-login kick messages are resolved in the language of the joining player, identified
        // here by their unique id since no Player object exists yet.
        given(messages.retrieveSingle(uniqueId, name, exception.getReason(), exception.getArgs()))
            .willReturn(message);
    ...
    /**
     * Returns a real {@link InetAddress} for the given numeric IP.
     * <p>
     * This used to be a Mockito mock, which stopped working on Java 16 and later: {@code
     * java.net.InetAddress} became a sealed class there and Mockito refuses to mock or spy it.
     * Since only {@link InetAddress#getHostAddress()} is needed, a genuine address built from the
     * IP literal serves the same purpose and is version independent. The address is constructed
     * from its raw bytes so that no name resolution is ever attempted.
     *
     * @param ip the numeric IPv4 address, e.g. {@code "12.34.56.78"}
     * @return an address whose {@code getHostAddress()} returns the given IP
     */
    private static InetAddress mockAddrWithIp(String ip) {
        ...
        try {
            ...
            throw new IllegalStateException("Invalid test IP address '" + ip + "'", e);
        }
    }
}
